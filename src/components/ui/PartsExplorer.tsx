...
export function PartsExplorer() {
  ...
  return (
    <div
      className={`fixed left-6 top-24 z-40 transition-transform duration-300 ${!isOpen ? "-translate-x-[calc(100%+2rem)]" : ""}`}
    >
      <button
          ...
          {MODEL_PARTS.map((part) => (
            ...
            >
              <div className="flex items-start justify-between mb-2">
                <h3
                  className={`font-medium ${selectedPart === part.id ? "text-text-primary" : "text-text-secondary"}`}
                >
                  {part.name}
                </h3>
                <span
